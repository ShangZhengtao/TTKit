...
@property (nonatomic, strong) UIColor *SXAnotherColor;
@property (nonatomic, strong) UIFont *SXAnotherFont;
...
- (void)setColorText:(NSString *)text{
    if ([text rangeOfString:@"<"].location != NSNotFound){
        if (!self.SXAnotherColor) {
            self.SXAnotherColor = [UIColor redColor];
        }
...
- (void)setFontText:(NSString *)text{
    if ([text rangeOfString:@"["].location != NSNotFound) {
        if (!self.SXAnotherFont) {
            self.SXAnotherFont = [UIFont boldSystemFontOfSize:18];
        }
...
- (void)setAnotherColor:(UIColor *)color {
    self.SXAnotherColor = color;
}

- (void)setAnotherFont:(UIFont *)font {
    self.SXAnotherFont = font;
}
...
- (NSMutableAttributedString *)addAttributeString:(NSMutableAttributedString *)attributeStr withArray:(NSArray *)dictArray type:(SXLabelType)type{
    ...
    NSObject *value = type == SXLabelTypeColor ? self.SXAnotherColor : self.SXAnotherFont;
...
- (void)setSXAnotherFont:(UIFont *)SXAnotherFont {
    if (SXAnotherFont == nil ) {
        SXAnotherFont = [UIFont systemFontOfSize:14];
    }
    objc_setAssociatedObject(self, @selector(SXAnotherFont), SXAnotherFont, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIFont *)SXAnotherFont {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setSXAnotherColor:(UIColor *)SXAnotherColor {
    if (SXAnotherColor == nil) {
        SXAnotherColor = [UIColor redColor];
    }
    objc_setAssociatedObject(self, @selector(SXAnotherColor), SXAnotherColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIColor *)SXAnotherColor {
    return objc_getAssociatedObject(self, _cmd);
}
